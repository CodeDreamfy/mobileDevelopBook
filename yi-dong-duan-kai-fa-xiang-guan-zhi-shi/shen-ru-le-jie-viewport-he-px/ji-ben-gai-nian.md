...
比如设备分辨率750\*1334为什么在chrome下面显示为375\*667？这是两种像素定义：一种是指屏幕可以达到的最小单位（硬件像素），另一种是指视觉上一致的“参照像素”
...
也叫做**密度无关像素**，可以认为是计算机坐标系统中的一个点，这个点代表一个可以**由程序使用的虚拟像素**（比如css像素），然后由相关系统转化为物理像素。（也就是css像素，可以转化为物理像素）
